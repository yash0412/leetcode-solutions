...
function minimumIndex(nums) {
    let dominantElement = nums[0], count = 0, prefixSum = 0;
    ...
    const totalCount = nums.reduce((acc, num) => (num === dominantElement ? acc + 1 : acc), 0);
...
function isValidSplit(splitIndex, prefixSum, domCount, totalElements) {
    const leftCount = prefixSum;
    const rightCount = domCount - leftCount;
    return (isDominant(leftCount, splitIndex + 1) &&
        isDominant(rightCount, totalElements - splitIndex - 1));
}
function isDominant(domCount, totalElements) {
